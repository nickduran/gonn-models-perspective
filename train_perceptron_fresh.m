...
% initial training, initial ~2500 trials (see Figure 5a) 
...
function [wih, who, biash, biaso, err, cosne, outacts] = train_perceptron_fresh(sents,targs,hids,numpres,lr)
...
for epochs=1:numpres,
    ...
    who = who + dwho;% + (rand(hids,outs) - .5)*err(end)/(4*hids^2);
    wih = wih + dwih;% + (rand(ins,hids) - .5)*err(end)/(4*hids^2);
